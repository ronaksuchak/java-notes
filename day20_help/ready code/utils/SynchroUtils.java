...
public class SynchroUtils {
	private static Random r1 = new Random();//created instance of random no gen.

	public synchronized void test() {
		System.out.println("entered test " 
	+ Thread.currentThread().getName());
		try {
			Thread.sleep(r1.nextInt(40) + 100);//random no gen : 100 ---139
	...
	public synchronized void testMe() {
		...
		try {
			Thread.sleep(r1.nextInt(100) + 20);//20---119
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void testMeAgain() {
		System.out.println("entered testMeagain "
				+ Thread.currentThread().getName());
		try {
			Thread.sleep(r1.nextInt(50) + 1);//1-50
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
