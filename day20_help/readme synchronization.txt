...
To prevent race conditions from occurring you must make sure that the critical section is executed as an atomic instruction. It means that once a single thread is executing it, no other threads can execute it until the first thread has left the critical section.
...
Using synchronized keyword , a lock is applied at the object level.(shared resource)
...
Un blocked -- monitor free.(synchronized method rets or synchronized block over)
...
1. Instead of writing long synchronized methods (n thus reducing the performance due to larger extent of the lock) , identify exact critical section & guard it using synchronized block.
...
synchronized block syntax : typically appear within run()

synchronized(shared resource ref.)
{ 
Access the thread un safe (un synched) methods of shared resource in mutually exclusive manner.
}
